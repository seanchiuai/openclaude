 ...
 * - Bare `$VAR` (no braces) is NOT matched
...
// Backward-compatible aliases for existing callers (loader.ts, other tests)
export function substituteEnvVars(value: string): string {
  return substituteString(value, process.env, "");
}

export function substituteEnvVarsDeep(obj: unknown): unknown {
  return resolveConfigEnvVars(obj);
}
