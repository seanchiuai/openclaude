...
describe("memory manager readonly recovery", () => {
  ...
  function createMemoryConfig() {
    return buildTestMemoryConfig({ workspaceDir, indexPath });
  }
  ...
  beforeEach(async () => {
    resetEmbeddingMocks();
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaude-mem-readonly-"));
  ...
  it("reopens sqlite and retries when readonly appears in error code", async () => {
    // When err is a plain object (not an Error instance), String(err) yields "[object Object]"
    // but the readonly detection still works via the code field.
    await expectReadonlyRetry({
      firstError: { message: "write failed", code: "SQLITE_READONLY" },
      expectedLastError: "[object Object]",
    });
  });
