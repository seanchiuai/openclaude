...
// Extension-based MIME lookup for common memory file types.
...
/**
 * Detect MIME type from buffer magic bytes and file extension.
 * Inlined to avoid dependency on a full media/mime module.
 */
async function detectMime(opts: {
  buffer?: Buffer;
  filePath?: string;
}): Promise<string | undefined> {
  // Try magic-byte sniffing first
  if (opts.buffer) {
    try {
