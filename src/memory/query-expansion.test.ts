...
import { buildFtsQuery, bm25RankToScore, expandQueryForFts, extractKeywords } from "./query-expansion.js";
...
describe("buildFtsQuery", () => {
  it("builds AND query from meaningful tokens", () => {
    const result = buildFtsQuery("API deployment strategy");
    expect(result).toBe('"API" AND "deployment" AND "strategy"');
  });

  it("filters stop words", () => {
    const result = buildFtsQuery("the solution for the bug");
...
describe("bm25RankToScore", () => {
  ...
  it("converts positive rank to 0-1 score", () => {
