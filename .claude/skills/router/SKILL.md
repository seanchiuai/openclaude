...
2. Gateway commands (`/help`, `/list`, `/stop`, `/status`, `/skills`, `/memory`, `/cron`) → direct response, no spawn
...
- Subagent tools (`sessions_spawn`, `sessions_status`) in gateway tools section

### Gateway Commands with Subagent Support

- `/list` shows subagent tree under parent sessions: `└─ sub-xyz [running] "research" (1m 23s)`
- `/stop <id>` cascade-kills parent + all active children via `subagentRegistry`
- `CommandDeps` includes optional `subagentRegistry` for tree view and cascade kill
